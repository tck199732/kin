...
Int_t GetA(Double_t myZ, Double_t myAoq, Double_t sigZ = 0.5, Double_t sigA = 0.005) {
	...
	for (int iz = iZ1; iz <= iZ2; iz++) {
		for (int im = iM1; im <= iM2; im++) {
			...
			ellipse = (myZ - (double)iz) * (myZ - (double)iz) / sigZ / sigZ +
					  (myAoq - AQ) * (myAoq - AQ) / sigA / sigA; // the ellipse can be numerically changed here
			if (ellipse < 1.) {
				iZ = iz;
				iA = im;
				break;
...
int main(int argc, char *argv[]) {
	...
	double end_projection_z =
		-592.644; //////mid target = -592.644, start pad plane =-580.4, end of pad plane = 763.6//AC paddles (LR) -857.3

	Int_t maxEvents = 50000000;
	auto bigripsParameters = TArtBigRIPSParameters::Instance();
	if (ppacRun != -1)
		bigripsParameters->LoadParameter(Form("db/BigRIPSPPAC/BigRIPSPPAC.%d.xml", ppacRun));
	else
		bigripsParameters->LoadParameter((Char_t *)"db/BigRIPSPPAC.xml");
	if (plaRun != -1)
		bigripsParameters->LoadParameter(Form("db/BigRIPSPlastic/BigRIPSPlastic.%d.xml", plaRun));
	else
		bigripsParameters->LoadParameter((Char_t *)"db/BigRIPSPlastic.xml");
	if (icRun != -1)
		bigripsParameters->LoadParameter(Form("db/BigRIPSIC/BigRIPSIC.%d.xml", icRun));
	else
	...
	// Calculate the flight length of up to middle and middle to downstream focal plane (last argument is middle focal
	// plane) Load the focal plane parameters
	//////////////////////
	...
	auto *histAllBeamPID = new TH2D(
		"histAllBeamPID", "", 1000, goalAoQ - spreadAoQ, goalAoQ + spreadAoQ, 2000, goalZ - spreadZ, goalZ + spreadZ
	);
	...
	Bool_t F7FP_in;
	if (fill_cuts) {
		cut_tree->Branch("F3PLA_in", &F3PLA_in, "F3PLA_in/B");
		cut_tree->Branch("F7PLA_in", &F7PLA_in, "F7PLA_in/B");
		cut_tree->Branch("F13_1PLA_in", &F13_1PLA_in, "F13_1PLA_in/B");
		cut_tree->Branch("F13_2PLA_in", &F13_2PLA_in, "F13_2PLA_in/B");
		...
		cut_tree->Branch("F3FP_in", &F3FP_in, "F3FP_in/B");
		cut_tree->Branch("F5FP_in", &F5FP_in, "F5FP_in/B");
		cut_tree->Branch("F7FP_in", &F7FP_in, "F7FP_in/B");
	}
	...
	raw->Branch("F3PPAC1A_X", &F3PPAC1A_X, "F3PPAC1A_X/D");
	raw->Branch("F3PPAC1A_Y", &F3PPAC1A_Y, "F3PPAC1A_Y/D");
	raw->Branch("F3PPAC1B_X", &F3PPAC1B_X, "F3PPAC1B_X/D");
	raw->Branch("F3PPAC1B_Y", &F3PPAC1B_Y, "F3PPAC1B_Y/D");
	raw->Branch("F3PPAC2A_X", &F3PPAC2A_X, "F3PPAC2A_X/D");
	raw->Branch("F3PPAC2A_Y", &F3PPAC2A_Y, "F3PPAC2A_Y/D");
	raw->Branch("F3PPAC2B_X", &F3PPAC2B_X, "F3PPAC2B_X/D");
	raw->Branch("F3PPAC2B_Y", &F3PPAC2B_Y, "F3PPAC2B_Y/D");

	raw->Branch("F5PPAC1A_X", &F5PPAC1A_X, "F5PPAC1A_X/D");
	raw->Branch("F5PPAC1A_Y", &F5PPAC1A_Y, "F5PPAC1A_Y/D");
	raw->Branch("F5PPAC1B_X", &F5PPAC1B_X, "F5PPAC1B_X/D");
	raw->Branch("F5PPAC1B_Y", &F5PPAC1B_Y, "F5PPAC1B_Y/D");
	raw->Branch("F5PPAC2A_X", &F5PPAC2A_X, "F5PPAC2A_X/D");
	raw->Branch("F5PPAC2A_Y", &F5PPAC2A_Y, "F5PPAC2A_Y/D");
	raw->Branch("F5PPAC2B_X", &F5PPAC2B_X, "F5PPAC2B_X/D");
	raw->Branch("F5PPAC2B_Y", &F5PPAC2B_Y, "F5PPAC2B_Y/D");

	raw->Branch("F7PPAC1A_X", &F7PPAC1A_X, "F7PPAC1A_X/D");
	raw->Branch("F7PPAC1A_Y", &F7PPAC1A_Y, "F7PPAC1A_Y/D");
	raw->Branch("F7PPAC1B_X", &F7PPAC1B_X, "F7PPAC1B_X/D");
	raw->Branch("F7PPAC1B_Y", &F7PPAC1B_Y, "F7PPAC1B_Y/D");
	raw->Branch("F7PPAC2A_X", &F7PPAC2A_X, "F7PPAC2A_X/D");
	raw->Branch("F7PPAC2A_Y", &F7PPAC2A_Y, "F7PPAC2A_Y/D");
	raw->Branch("F7PPAC2B_X", &F7PPAC2B_X, "F7PPAC2B_X/D");
	raw->Branch("F7PPAC2B_Y", &F7PPAC2B_Y, "F7PPAC2B_Y/D");

	raw->Branch("ICQ", &ICQ, "ICQ[8]/I");

	raw->Branch("F3PLA_TL", &F3PLA_TL, "F3PLA_TL/D");
	raw->Branch("F3PLA_TR", &F3PLA_TR, "F3PLA_TR/D");
	raw->Branch("F3PLA_TL_slew_slew", &F3PLA_TL_slew, "F3PLA_TL_slew/D");
	raw->Branch("F3PLA_TR_slew", &F3PLA_TR_slew, "F3PLA_TR_slew/D");
	raw->Branch("F3PLA_QL", &F3PLA_QL, "F3PLA_QL/I");
	raw->Branch("F3PLA_QR", &F3PLA_QR, "F3PLA_QR/I");
	raw->Branch("F3PLA_DT", &F3PLA_DT, "F3PLA_DT/D");
	raw->Branch("F3PLA_Q_TEST", &F3PLA_Q_TEST, "F3PLA_Q_TEST/D");

	raw->Branch("F7PLA_TL", &F7PLA_TL, "F7PLA_TL/D");
	raw->Branch("F7PLA_TR", &F7PLA_TR, "F7PLA_TR/D");
	raw->Branch("F7PLA_TL_slew_slew", &F7PLA_TL_slew, "F7PLA_TL_slew/D");
	raw->Branch("F7PLA_TR_slew", &F7PLA_TR_slew, "F7PLA_TR_slew/D");
	raw->Branch("F7PLA_QL", &F7PLA_QL, "F7PLA_QL/I");
	raw->Branch("F7PLA_QR", &F7PLA_QR, "F7PLA_QR/I");
	raw->Branch("F7PLA_DT", &F7PLA_DT, "F7PLA_DT/D");
	raw->Branch("F7PLA_Q_TEST", &F7PLA_Q_TEST, "F7PLA_Q_TEST/D");

	raw->Branch("F13_1PLA_TL", &F13_1PLA_TL, "F13_1PLA_TL/D");
	raw->Branch("F13_1PLA_TR", &F13_1PLA_TR, "F13_1PLA_TR/D");
	raw->Branch("F13_1PLA_QL", &F13_1PLA_QL, "F13_1PLA_QL/I");
	raw->Branch("F13_1PLA_QR", &F13_1PLA_QR, "F13_1PLA_QR/I");
	raw->Branch("F13_1PLA_DT", &F13_1PLA_DT, "F13_1PLA_DT/D");
	raw->Branch("F13_1PLA_Q_TEST", &F13_1PLA_Q_TEST, "F13_1PLA_Q_TEST/D");

	raw->Branch("F13_2PLA_TL", &F13_2PLA_TL, "F13_2PLA_TL/D");
	raw->Branch("F13_2PLA_TR", &F13_2PLA_TR, "F13_2PLA_TR/D");
	raw->Branch("F13_2PLA_QL", &F13_2PLA_QL, "F13_2PLA_QL/I");
	raw->Branch("F13_2PLA_QR", &F13_2PLA_QR, "F13_2PLA_QR/I");
	raw->Branch("F13_2PLA_DT", &F13_2PLA_DT, "F13_2PLA_DT/D");
	raw->Branch("F13_2PLA_Q_TEST", &F13_2PLA_Q_TEST, "F13_2PLA_Q_TEST/D");
	...
	// TBeam branches
	Int_t neve = 0;
	TBeam->Branch("neve", &neve, "neve/I");
	Double_t z = -9999;
	TBeam->Branch("z", &z, "z/D");

	Double_t aoq = -9999;
	TBeam->Branch("aoq", &aoq, "aoq/D");
	Double_t beta37 = -9999;
	TBeam->Branch("beta37", &beta37, "beta37/D");
	Double_t brho78 = -9999;
	TBeam->Branch("brho78", &brho78, "brho78/D");
	Double_t beta = -9999;
	TBeam->Branch("beta", &beta, "beta/D");
	Double_t brho = -9999;
	TBeam->Branch("brho", &brho, "brho/D");
	Bool_t isGood = false;
	TBeam->Branch("isGood", &isGood, "isGood/B");
	Int_t intZ = -9999;
	TBeam->Branch("intZ", &intZ, "intZ/I");
	Int_t intA = -9999;
	TBeam->Branch("intA", &intA, "intA/I");
	Double_t length = -9999;
	TBeam->Branch("length", &length, "length/D");
	...
	Double_t bdcax = -9999;
	TBDC->Branch("bdcax", &bdcax, "bdcax/D");
	Double_t bdcby = -9999;
	TBDC->Branch("bdcby", &bdcby, "bdcby/D");
	...
	Double_t ProjZ = -9999;
	TBDC->Branch("ProjZ", &ProjZ, "ProjZ/D");
	Double_t ProjA = -9999;
	TBDC->Branch("ProjA", &ProjA, "ProjA/D");
	Double_t ProjB = -9999;
	TBDC->Branch("ProjB", &ProjB, "ProjB/D");
	Double_t ProjE = -9999;
	TBDC->Branch("ProjE", &ProjE, "ProjE/D");
	Double_t ProjMeVu = -9999;
	TBDC->Branch("ProjMeVu", &ProjMeVu, "ProjMeVu/D");
	Double_t ProjBeta = -9999;
	TBDC->Branch("ProjBeta", &ProjBeta, "ProjBeta/D");
	Double_t ProjP = -9999;
	TBDC->Branch("ProjP", &ProjP, "ProjP/D");
	Double_t ProjPX = -9999;
	TBDC->Branch("ProjPX", &ProjPX, "ProjPX/D");
	Double_t ProjPY = -9999;
	TBDC->Branch("ProjPY", &ProjPY, "ProjPY/D");
	Double_t ProjPZ = -9999;
	...
	*/
	...
	TFile *bdcin = nullptr;
	if (dctpfRun != -1)
		bdcin = new TFile(Form("./dctpf/dc_tpf_%d.root", dctpfRun), "READ");
	else
		bdcin = new TFile("./dctpf/dc_tpf.root", "READ");
	if (bdcin->IsOpen()) {
		if (dctpfRun != -1)
	...
	while (eventStore->GetNextEvent() && numEvents < maxEvents) {
		if (numEvents % 100 == 0) {
			std::cout << "\r event: " << numEvents << " / " << maxEvents << " (" << 100. * numEvents / maxEvents << "%)"
					  << std::flush;
		}
		...
		for (int i = 0; i < rawevent->GetNumSeg(); i++) {
			...
			if (detector == 1) { // PPACT
				...
				for (int j = 0; j < seg->GetNumData(); j++) {
					...
					if (ch == 12)
						nppachit_f7_1a_anode++;
					if (ch == 13)
						nppachit_f7_1b_anode++;
					if (ch == 14)
						nppachit_f7_2a_anode++;
					if (ch == 15)
						nppachit_f7_2b_anode++;
		...
		if (z > 0 && z < 75 && aoq > 1. && aoq < 3 && bdc1x > -999 && bdc1y > -999 && bdc2x > -999 && bdc2y > -999) {
			bdcProj->ProjectParticle(
				bdc2x, bdc2y, -2160., bdcax, bdcby, z, E1, end_projection_z, beamE->getMass()
			); //-580.4,-583.904
			ProjX = bdcProj->getX();
			ProjY = bdcProj->getY();
			ProjZ = bdcProj->getZ();
		...
		/*
		  ...
		  bdcProj->ProjectParticle(bdc1x, bdc1y, -3160., bdc1a*2000./TMath::Pi(), bdc1b*2000./TMath::Pi(), z, E1,
		end_projection_z,beamE->getMass());//-580.4,-583.904 ProjA1=bdc1a; ProjB1=bdc1b;
		  }
		  if(bdc2x>-999 && bdc2y>-999){
		  bdcProj->ProjectParticle(bdc2x, bdc2y, -2160., bdc2a*2000./TMath::Pi(), bdc2b*2000./TMath::Pi(), z, E1,
		end_projection_z,beamE->getMass());//-580.4,-583.904 ProjA1=bdc2a; ProjB1=bdc2b;
		  }
		...
		*/
		...
		F7PLA_Q_TEST = -9999;
		F13_1PLA_DT = -9999;
		F13_1PLA_Q_TEST = -9999;
		F13_2PLA_DT = -9999;
		F13_2PLA_Q_TEST = -9999;
		...
		if (ppac) {
			F3PPAC1A_X = ppac->GetX();
			F3PPAC1A_Y = ppac->GetY();
		}
		ppac = calibPPAC->FindPPAC((Char_t *)"F3PPAC-1B");
		if (ppac) {
			F3PPAC1B_X = ppac->GetX();
			F3PPAC1B_Y = ppac->GetY();
		}
		ppac = calibPPAC->FindPPAC((Char_t *)"F3PPAC-2A");
		if (ppac) {
			F3PPAC2A_X = ppac->GetX();
			F3PPAC2A_Y = ppac->GetY();
		}
		ppac = calibPPAC->FindPPAC((Char_t *)"F3PPAC-2B");
		if (ppac) {
			F3PPAC2B_X = ppac->GetX();
			F3PPAC2B_Y = ppac->GetY();
		}

		ppac = calibPPAC->FindPPAC((Char_t *)"F5PPAC-1A");
		if (ppac) {
			F5PPAC1A_X = ppac->GetX();
			F5PPAC1A_Y = ppac->GetY();
		}
		ppac = calibPPAC->FindPPAC((Char_t *)"F5PPAC-1B");
		if (ppac) {
			F5PPAC1B_X = ppac->GetX();
			F5PPAC1B_Y = ppac->GetY();
		}
		ppac = calibPPAC->FindPPAC((Char_t *)"F5PPAC-2A");
		if (ppac) {
			F5PPAC2A_X = ppac->GetX();
			F5PPAC2A_Y = ppac->GetY();
		}
		ppac = calibPPAC->FindPPAC((Char_t *)"F5PPAC-2B");
		if (ppac) {
			F5PPAC2B_X = ppac->GetX();
			F5PPAC2B_Y = ppac->GetY();
		}

		ppac = calibPPAC->FindPPAC((Char_t *)"F7PPAC-1A");
		if (ppac) {
			F7PPAC1A_X = ppac->GetX();
			F7PPAC1A_Y = ppac->GetY();
		}
		ppac = calibPPAC->FindPPAC((Char_t *)"F7PPAC-1B");
		if (ppac) {
			F7PPAC1B_X = ppac->GetX();
			F7PPAC1B_Y = ppac->GetY();
		}
		ppac = calibPPAC->FindPPAC((Char_t *)"F7PPAC-2A");
		if (ppac) {
			F7PPAC2A_X = ppac->GetX();
			F7PPAC2A_Y = ppac->GetY();
		}
		ppac = calibPPAC->FindPPAC((Char_t *)"F7PPAC-2B");
		if (ppac) {
			F7PPAC2B_X = ppac->GetX();
			F7PPAC2B_Y = ppac->GetY();
		}

		TArtPlastic *pla;
		pla = calibPLA->FindPlastic((Char_t *)"F3pl");
		if (pla) {
			F3PLA_TL_slew = pla->GetTimeLSlew();
			F3PLA_TR_slew = pla->GetTimeRSlew();
			F3PLA_TL = pla->GetTLRaw();
			F3PLA_TR = pla->GetTRRaw();
			F3PLA_QL = pla->GetQLRaw();
			F3PLA_QR = pla->GetQRRaw();
			F3PLA_DT = F3PLA_TL - F3PLA_TR;
			F3PLA_Q_TEST = std::log(F3PLA_QR) - std::log(F3PLA_QL);
		}
		pla = calibPLA->FindPlastic((Char_t *)"F7pl");
		if (pla) {
			F7PLA_TL_slew = pla->GetTimeLSlew();
			F7PLA_TR_slew = pla->GetTimeRSlew();
			F7PLA_TL = pla->GetTLRaw();
			F7PLA_TR = pla->GetTRRaw();
			F7PLA_QL = pla->GetQLRaw();
			F7PLA_QR = pla->GetQRRaw();
			F7PLA_DT = F7PLA_TL - F7PLA_TR;
			F7PLA_Q_TEST = std::log(F7PLA_QR) - std::log(F7PLA_QL);
		}
		pla = calibPLA->FindPlastic((Char_t *)"F13pl-1");
		if (pla) {
			F13_1PLA_TL = pla->GetTLRaw();
			F13_1PLA_TR = pla->GetTRRaw();
			F13_1PLA_QL = pla->GetQLRaw();
			F13_1PLA_QR = pla->GetQRRaw();
			F13_1PLA_DT = F13_1PLA_TL - F13_1PLA_TR;
			F13_1PLA_Q_TEST = std::log(F13_1PLA_QR) - std::log(F13_1PLA_QL);
		}
		pla = calibPLA->FindPlastic((Char_t *)"F13pl-2");
		if (pla) {
			F13_2PLA_TL = pla->GetTLRaw();
			F13_2PLA_TR = pla->GetTRRaw();
			F13_2PLA_QL = pla->GetQLRaw();
			F13_2PLA_QR = pla->GetQRRaw();
			F13_2PLA_DT = F13_2PLA_TL - F13_2PLA_TR;
			F13_2PLA_Q_TEST = std::log(F13_2PLA_QR) - std::log(F13_2PLA_QL);
		}
		...
		F13_1PLA_in = true;
		F13_2PLA_in = true;
		totalPPAChits_good = true;
		F7A_PPAChits_lte8 = true;
		F7B_PPAChits_lte8 = true;
		/*
		...
		*/
		if (nppachit > 64 || nppachit < 50) {
			isGood = false;
			totalPPAChits_good = false;
		}
		if (nppachit_f7_0 > 8) {
			isGood = false;
			F7A_PPAChits_lte8 = false;
		}
		if (nppachit_f7_1 > 8) {
			isGood = false;
			F7B_PPAChits_lte8 = false;
		}
