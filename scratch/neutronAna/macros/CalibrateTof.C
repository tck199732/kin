...
void CalibrateTof(
	...
	const std::string &outputFilename = "tofVsModuleID.root",
	...
	auto hyPosVsxPos = new TH2D("hyPosVsxPos", "", 500, -1000, 1000, 500, -1000, 1000);
	TH2D *hyPosVsxPosLayer[3];
	for (int i = 0; i < 3; i++) {
		hyPosVsxPosLayer[i] = new TH2D(Form("hyPosVsxPosLayer%d", i), "", 500, -1000, 1000, 500, -1000, 1000);
	}
	...
	for (auto iEvt = 0; iEvt < nEntries; iEvt++) {
		...
		for (auto ihit = 0; ihit < spirit.hime_nHits; ihit++) {
			...
			hyPosVsxPos->Fill(xHit, yHit);
			hyPosVsxPosLayer[layerId]->Fill(xHit, yHit);
			...
			auto tof = spirit.hime_tofRaw[ihit] - (distanceHit - distance) / C_LIGHT;
