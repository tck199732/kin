...
void DrawBarHitPattern(
	// clang-format off
	const std::string &shadow_bar_pos = "before",
	const std::string &outputFilename = "bar_hit_pattern.root",
	...
	auto hHitPatternLayer0 = new TH2D("hHitPatternLayer0", "", 200, -1000, 1000, 24, 0, 24);
	auto hHitPatternLayer1 = new TH2D("hHitPatternLayer1", "", 24, 24, 48, 200, -1000, 1000);
	auto hHitPatternLayer2 = new TH2D("hHitPatternLayer2", "", 200, -1000, 1000, 24, 48, 72);
	...
	for (auto iEvt = 0; iEvt < nEntries; iEvt++) {
		...
		for (auto ihit = 0; ihit < spirit.hime_nHits; ihit++) {
			...
			if (layerId == 0) {
				hHitPatternLayer0->Fill(xHit, moduleId);
			} else if (layerId == 1) {
				hHitPatternLayer1->Fill(moduleId, yHit);
			} else if (layerId == 2) {
				hHitPatternLayer2->Fill(xHit, moduleId);
			}
		}
	}
	auto ofile = new TFile(outputFilename.c_str(), "recreate");

	hHitPatternLayer0->Scale(1. / nEntries);
	hHitPatternLayer1->Scale(1. / nEntries);
	hHitPatternLayer2->Scale(1. / nEntries);

	hHitPatternLayer0->Write();
	hHitPatternLayer1->Write();
	hHitPatternLayer2->Write();
