...
// clang-format off
const std::vector<int> shadow_modules_before = {
    6, 7, 16, 17, 42, 43, 54, 55, 64, 65,
};

const std::vector<int> shadow_modules_moved = {
    16, 17, 31, 32, 42, 43, 64, 65,
};

const std::vector<int> shadow_modules_final = {
    16, 17, 31, 32, 64, 65
};
// clang-format on

void DrawShadowCounts(
	// clang-format off
	const std::string &shadow_bar_pos = "before",
	const std::vector<double> &tot_cut = {0, 150}, 
    bool useVetoWall = false,
	const std::string &outputFilename = "shadow_counts.root",
    const int& pos_bins = 200,
	...
	std::vector<int> interested_modules;

	if (shadow_bar_pos == "before") {
		runIds.insert(runIds.end(), runIds_before.begin(), runIds_before.end());
		interested_modules.insert(interested_modules.end(), shadow_modules_before.begin(), shadow_modules_before.end());
	} else if (shadow_bar_pos == "moved") {
		runIds.insert(runIds.end(), runIds_moved.begin(), runIds_moved.end());
		interested_modules.insert(interested_modules.end(), shadow_modules_moved.begin(), shadow_modules_moved.end());
	} else if (shadow_bar_pos == "final") {
		runIds.insert(runIds.end(), runIds_final.begin(), runIds_final.end());
		interested_modules.insert(interested_modules.end(), shadow_modules_final.begin(), shadow_modules_final.end());
	} else {
	...
	std::map<int, TH1D *> hShadowCounts;
	for (auto moduleId : interested_modules) {
		hShadowCounts[moduleId] = new TH1D(Form("hShadowCounts_%d", moduleId), "", pos_bins, -1000, 1000);
	}
	...
	for (auto iEvt = 0; iEvt < nEntries; iEvt++) {
		...
		for (auto ihit = 0; ihit < spirit.hime_nHits; ihit++) {
			...
			if (hShadowCounts.count(moduleId) == 0) {
				continue;
			}
			...
			if (tot < tot_cut[0] || tot > tot_cut[1]) {
				continue;
			}
			...
			if (layerId == 0) {
				hShadowCounts[moduleId]->Fill(xHit);
			} else if (layerId == 1) {
				hShadowCounts[moduleId]->Fill(yHit);
			} else if (layerId == 2) {
				hShadowCounts[moduleId]->Fill(xHit);
			}
		}
	...
	for (auto moduleId : interested_modules) {
		hShadowCounts[moduleId]->Scale(1. / nEntries);
		hShadowCounts[moduleId]->Write();
	}
	ofile->Close();
