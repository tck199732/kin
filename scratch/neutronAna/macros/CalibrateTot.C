...
void CalibrateTot(
	...
	const std::string &outputFilename = "tot_vs_tof_cw_noveto.root",
    ...
    bool applyCrossWalk = true,
    const double& crosswalkThreshold = 23,
    const double& crosswalkSlope = -0.197618,
    const double& crosswalkIntercept = -6.45002
	// clang-format on
) {
	...
	for (auto iEvt = 0; iEvt < nEntries; iEvt++) {
		...
		for (auto ihit = 0; ihit < spirit.hime_nHits; ihit++) {
			...
			if (applyCrossWalk) {
				if (tot < crosswalkThreshold) {
					tof -= (crosswalkSlope * tot + crosswalkIntercept);
				} else {
					tof -= (crosswalkSlope * crosswalkThreshold + crosswalkIntercept);
				}
			}
