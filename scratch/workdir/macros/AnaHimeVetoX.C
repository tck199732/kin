...
void AnaHimeVetoX(
	...
	const std::string& filenameOut = "results-kin/hime-veto-X-check.root",
	...
	TH2D *hHitCorrelation[3];
	for (auto ibar = 0; ibar < 3; ibar++) {
		hHitCorrelation[ibar] = new TH2D(Form("hHitCorrelation%d", ibar), "", 250, -1000, 1000, 250, -1000, 1000);
	}

	auto hHitPatternLayer0 = new TH2D("hHitPatternLayer0", "", 1000, -1000, 1000, 1000, -1000, 1000);
	...
	for (auto iEvt = 0; iEvt < nEvents; iEvt++) {
		...
		for (auto h = 0; h < spirit.hime_nHits; h++) {
			...
			auto tDiffOffset = calibVelocity[hitModule] != 0.0 ? posOffset[hitModule] / calibVelocity[hitModule] : 0.0;
			auto tDiff = spirit.hime_tDiff[h] + tDiffOffset; // ns
			...
			double yhit = 0.5 * barGap + deltaModule * (barWidth + barGap) + barWidth * unif(gen);

			for (auto vhit = 0; vhit < spirit.hime_veto_multi; vhit++) {
				int bar = spirit.hime_veto_bar[vhit];
				auto xVeto = spirit.hime_veto_x[vhit];
				auto tdiff = spirit.hime_veto_tdiff[vhit];
				auto charge = spirit.hime_veto_charge[vhit];
				auto tof = spirit.hime_veto_tof[vhit];
				hHitCorrelation[bar]->Fill(barPos, xVeto);
			}
			// as a check
			hHitPatternLayer0->Fill(barPos, yhit);
		}
	}

	// normalization
	for (auto ibar = 0; ibar < 3; ibar++) {
		hHitCorrelation[ibar]->Scale(1.0 / nEvents);
	}
	hHitPatternLayer0->Scale(1.0 / nEvents);
	auto fileOut = new TFile(filenameOut.c_str(), "RECREATE");
	for (auto ibar = 0; ibar < 3; ibar++) {
		hHitCorrelation[ibar]->Write();
	}
	hHitPatternLayer0->Write();
	fileOut->Close();
	return;
}
